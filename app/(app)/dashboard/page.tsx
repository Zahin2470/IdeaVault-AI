...
// unlike the Idea Vault / Task Board pages — the stagger/count-up motion
// is delegated to small client components rather than making the whole
    ...
    <div className="flex flex-col gap-8">
      ...
      <section className="flex flex-col gap-3">
        <div className="flex items-center justify-between">
          ...
          <Link href="/projects" className="text-sm text-accent transition-premium hover:opacity-75">
            View all
          </Link>
        </div>
            ...
            <div className="flex flex-col divide-y divide-border rounded-lg border border-border">
              {upcomingTasks.map((t: {
                ...
                <Link
                  ...
                  className="flex items-center justify-between p-3 text-sm transition-premium hover:bg-muted"
                >
                  <div>
                    <p>{t.title}</p>
            ...
            <div className="flex flex-col divide-y divide-border rounded-lg border border-border">
              {favoriteIdeas.map((idea: { id: string; title: string }) => (
                <Link
                  key={idea.id}
                  href="/ideas"
                  className="flex items-center gap-2 p-3 text-sm transition-premium hover:bg-muted"
                >
...
function EmptyRow({ text, href, cta }: { text: string; href: string; cta: string }) {
  return (
    <div className="flex items-center justify-between rounded-lg border border-dashed border-border p-4 text-sm text-muted-foreground">
      <span>{text}</span>
      <Link href={href} className="flex items-center gap-1 text-accent transition-premium hover:opacity-75">
